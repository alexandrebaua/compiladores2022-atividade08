...
{
    /// <summary>
    /// Classe para o comando do tipo 'Enquanto-Faça'.
    /// </summary>
    public class EnquantoFaça : Tipo
    {
        /// <summary>
        ...
        public EnquantoFaça(List<SemanticoToken> elementos, List<ITipo> listaTiposEscopoMaior)
        {
            if (elementos == null)
                throw new ArgumentNullException(nameof(elementos));
            if (elementos.Count < 5)
                throw new Exception("Um tipo 'enquanto-faça' deve receber no mínimo 5 elementos!");
        ...
        {
            mips.SectionText.Adicionar(new MipsText($"enquantoIni_{this.Elementos[0].Index}:", null));

            this.ExpressãoLógica.GerarMips(mips, VariávelTipo.Lógico);
            mips.SectionText.Adicionar(new MipsText("beq", $"$t4,1,enquanto_{this.Elementos[0].Index}"));

            mips.SectionText.Adicionar(new MipsText("j", $"enquantoFim_{this.Elementos[0].Index}"));
            mips.SectionText.Adicionar(new MipsText($"enquanto_{this.Elementos[0].Index}:", null));

            foreach (var comando in this.ListaTiposEscopoMaior)
                comando.GerarMips(mips);

            mips.SectionText.Adicionar(new MipsText("j", $"enquantoIni_{this.Elementos[0].Index}"));
            mips.SectionText.Adicionar(new MipsText($"enquantoFim_{this.Elementos[0].Index}:", null));
        ...
        public override string ToString()
        {
            ...
            return $"Enquanto-Faça: '{argLogico}', {(this.ListaTiposEscopoMaior == null ? 0 : this.ListaTiposEscopoMaior.Count())} comandos";
        }
    }
}
